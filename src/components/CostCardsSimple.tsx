...
const CostCardsSimple = ({
  prices,
  rollingPrices
}: CostCardsSimpleProps) => {
  ...
  return <>
      <div className="mb-6 sm:mb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 max-w-5xl mx-auto">
          ...
          return <button key={category.id} onClick={() => handleCardClick(category)} className="bg-card rounded-xl shadow-card p-3 sm:p-4 lg:p-5 border border-border hover:border-primary hover:shadow-glow transition-all duration-300 cursor-pointer text-left group">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
          ...
          {selectedCategory && getRecommendedTime() && <div className="space-y-3 sm:space-y-4">
              ...
              <div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div>
                    ...
                    <Slider id="kwh-slider" min={selectedCategory.kWhRange[0]} max={selectedCategory.kWhRange[1]} step={selectedCategory.kWhRange[1] > 10 ? 5 : 0.5} value={[customKwh]} onValueChange={value => setCustomKwh(value[0])} className="w-full" />
                  </div>
                  ...
                  <div>
                    ...
                    <Slider id="start-hour-slider" min={0} max={Math.max(0, rollingPrices.length - customDuration)} step={1} value={[customStartHour]} onValueChange={value => setCustomStartHour(value[0])} className="w-full" />
                  </div>
                  ...
                  <div>
                    ...
                    <Slider id="duration-slider" min={1} max={Math.min(12, rollingPrices.length - customStartHour)} step={1} value={[customDuration]} onValueChange={value => setCustomDuration(value[0])} className="w-full" />
                  </div>

                  {calculateCustomCost() && <div className="bg-muted/50 rounded-lg p-3 sm:p-4 border border-border">
                      <div className="space-y-1.5 sm:space-y-2">
                        ...
                        {calculateCustomCost()!.savings > 0 && <div className="flex justify-between items-center text-price-cheap-foreground">
                            <span className="text-xs sm:text-sm text-secondary-foreground">Besparing:</span>
