...
const PriceHighLowCards = ({ prices, cheapest4Window, rollingPrices }: PriceHighLowCardsProps) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
      ...
      <div className="bg-gradient-to-br from-emerald-100 to-emerald-200 dark:from-emerald-950 dark:to-emerald-900 rounded-lg shadow-card p-4 border border-emerald-300 dark:border-emerald-700">
        <div className="flex items-center gap-3 mb-3">
          <div className="p-2 bg-emerald-700/30 dark:bg-emerald-600/20 rounded-lg">
            <TrendingDown className="w-5 h-5 text-emerald-900 dark:text-emerald-300" />
          </div>
          <div>
            <p className="text-xs text-emerald-800 dark:text-emerald-200 font-medium">Dagens lägsta pris</p>
            <p className="text-sm font-semibold text-emerald-950 dark:text-emerald-50">
              Klockan {formatHour(lowestPrice.hour)}
            </p>
          </div>
        </div>
        <div className="text-2xl sm:text-3xl font-bold text-emerald-950 dark:text-emerald-50">
          {formatPrice(lowestPrice.price)}
        </div>
        <p className="text-xs text-emerald-700 dark:text-emerald-300 mt-1 font-medium">(inkl. moms)</p>
      </div>
    </div>
  );
};
